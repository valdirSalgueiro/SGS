...
#ifdef WIN32
		/*wcsound_start();
		explosion=new sound("../res/explosion.wav",25,100,50);
		aliendie=new sound("../res/aliendie1.wav",25,100,50);
...
#else
		explosionSND=JNIUtil::jni_sound_load("explosion.ogg");
		aliendie=JNIUtil::jni_sound_load("aliendie1.ogg");
		warning=JNIUtil::jni_sound_load("warning.ogg");
		alien1=JNIUtil::jni_sound_load("alien1.ogg");
		naveSound=JNIUtil::jni_sound_load("ship.ogg");
		alienTiro1=JNIUtil::jni_sound_load("alienTiro1.ogg");
#endif
...
void Engine::update(float time){
	...
	if(gameState==LOGO1){
		...
		if(touch){
			#ifndef WIN32
				JNIUtil::jni_music_play("level1.ogg");
			#endif
			gameState=PLAYING;
			touch=false;
		}
		...
		if(!started){
			...
			if(!naveSoundPlayed && frames>1900){ //nave some em 5800
				...
				//naveSound->play();
#else
				JNIUtil::jni_sound_play(naveSound);
#endif
					...
					for (size_t j=0;j<playerObjs.size();j++){
						...
						if(enemies[i]->Aabb.Intersects(aabb)){						
							if(enemies[i]->energy>0){
								...
								//								explosion->play();
#else
								JNIUtil::jni_sound_play(explosionSND);
#endif
								...
								//								aliendie->play();
#else
								JNIUtil::jni_sound_play(aliendie);
#endif
								enemies[i]->cleanUp();
		...
		if(boss!=0 && frames-bossTime<300){
			...
			if(frames%30<15){
				...
				if(!warningPlayed){
					...
					//					warning->play();
#else
					JNIUtil::jni_sound_play(warning);
#endif
					warningPlayed=true;
				}
				#ifdef USE_FONT
